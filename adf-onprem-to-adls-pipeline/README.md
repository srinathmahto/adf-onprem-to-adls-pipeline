...
2. Child Pipeline (PL_Child_Worker)
   ...
   - Upon successful copy, triggers the watermark update pipeline

3. Update Watermark Pipeline (PL_Update_Watermark)
   - Executes a stored procedure to update the control table
   - Locks in the new high-watermark threshold for incremental loads  
